...
from crypto_VDF.utils.utils import arrange_powers_of_2, get_current_file_abs_path, create_path_to_data_folder
import numpy as np
import pandas as pd
import os
...
class Grapher:
    ...
    def generate_pietrzak_complexity_data(number_of_delays=10,fname=None, delay_repeat=1, randomize_input=False):
        ...
        for i in delays_list:
            ...
            for repeat_time in range(delay_repeat):
                ...
                tProofEnd = t() - tProofStart 
                # print(f"proof took: {tProofEnd} seconds")
                yTimeEval = np.append(yTimeEval, tOutEnd+tProofEnd)
    ...
    def collect_pietrzak_complexity_data(number_of_delays,iterations):
        ...
        fixed_input_data = Grapher.generate_pietrzak_complexity_data(number_of_delays=number_of_delays,
                                                                     fname=fixed_input_file_path,delay_repeat=iterations,
        ...
        random_input_data = Grapher.generate_pietrzak_complexity_data(number_of_delays=number_of_delays,
                                                                      fname=random_input_file_path,delay_repeat=iterations,
        ...
        for delay in delays_list:
            fixed_input_eval_time_means = np.append(fixed_input_eval_time_means,
                                        fixed_input_data.loc[fixed_input_data['delay'] == delay]['eval time (s)'].sum()/iterations)
            fixed_input_verify_time_means = np.append(fixed_input_verify_time_means, 
                                        fixed_input_data.loc[fixed_input_data['delay'] == delay]['verify time (s)'].sum()/iterations)

            random_input_eval_time_means = np.append(random_input_eval_time_means,
                                        random_input_data.loc[random_input_data['delay'] == delay]['eval time (s)'].sum()/iterations)
            random_input_verify_time_means = np.append(random_input_verify_time_means, 
                                        random_input_data.loc[random_input_data['delay'] == delay]['verify time (s)'].sum()/iterations)


        fixed_input_data = pd.DataFrame({"delay":delays_list, f"eval time means for {iterations} iterations (s)": random_input_eval_time_means,
                 f"verify time means for {iterations} iterations (s)":fixed_input_verify_time_means, "input": fixed_input_data['input'].unique()})
